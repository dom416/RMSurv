...
from Hmodel.discrete_hazards_plot import plot_survival_probabilities
...
def train(opt,train_data,test_data,device):
    ...
    random.seed(666)
    model = HFBSurv(4, (48, 256), (20, 0, 20), (0.05, 0.3), 20, 0.1).to(device) 
    model.to(device) 
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter+50+1)):
        ...
        if opt.measure or epoch == (opt.niter+opt.niter_decay - 1):
            ...
            loss_test, cindex_test, _, _ = test(opt, model, test_data, device)
