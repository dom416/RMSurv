...
def train_mirna(opt,train_data,test_data,device):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=0.00012, betas=(opt.beta1, opt.beta2), weight_decay=opt.weight_decay)
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter+50+1)):
        ...
        if opt.measure or epoch == (opt.niter+opt.niter_decay - 1):
            ...
            loss_test, cindex_test, _, _ = test_mirna(opt, model, test_data, device)
...
def test_mirna(opt,model, test_data, device):
    model.eval()
