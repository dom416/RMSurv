...
def train_path(opt,train_data,test_data,device):
    cudnn.deterministic = True
    torch.cuda.manual_seed_all(666)
    torch.manual_seed(666)
    random.seed(666)
    model = HFBSurv(3584, (48, 64), (20, 0, 20), (0.01, 0.3), 20, 0.1).to(device)
    model.to(device) 
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter+opt.niter_decay+1)):
        ...
        if opt.measure or epoch == (opt.niter+opt.niter_decay - 1):
            ...
            loss_test, cindex_test, _, _ = test_path(opt, model, test_data, device)
...
def test_path(opt,model, test_data, device):
    model.eval()
...
def test_and_plot_v2(opt,model, test_data, device):
    model.eval()
    test_loader = DataLoader(dataset=test_data, batch_size=len(test_data), shuffle=False)#,pin_memory=True,num_workers=10,collate_fn=custom_collate_fn)
    risk_pred_all, censor_all, survtime_all = np.array([]), np.array([]),np.array([])
    loss_test = 0
