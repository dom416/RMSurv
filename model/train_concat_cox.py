...
def train_concat_cox(opt,train_data,test_data,device):
    cudnn.deterministic = True
    torch.cuda.manual_seed_all(666)
    torch.manual_seed(666)
    random.seed(666)
    #model = HFBSurv(27564, (256, 256), (20, 0, 1), (0.05, 0.3), 20, 0.1).to(device) 
    model = HFBSurv(4600, (256, 256), (20, 0, 1), (0.05, 0.3), 20, 0.1).to(device) 
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter+50+1)):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            fusion = torch.cat((x_clin,x_pathr,x_mirna),2)
        ...
        if opt.measure or epoch == (opt.niter+opt.niter_decay - 1):
            ...
            loss_test, cindex_test = test_concat_cox(opt, model, test_data, device)
...
def test_concat_cox(opt,model, test_data, device):
    model.eval()
