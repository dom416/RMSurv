...
def train_concat(opt,train_data,test_data,device,num_modalities):
    ...
    if num_modalities == 1:
        model = HFBSurv(4, (256, 256), (20, 0, 20), (0.05, 0.3), 20, 0.1).to(device)
    if num_modalities == 2:
        model = HFBSurv(1016, (256, 256), (20, 0, 20), (0.05, 0.3), 20, 0.1).to(device)
    if num_modalities == 3:
        model = HFBSurv(4600, (256, 256), (20, 0, 20), (0.05, 0.3), 20, 0.1).to(device)
    if num_modalities == 4:
        model = HFBSurv(9531, (256, 256), (20, 0, 20), (0.05, 0.3), 20, 0.1).to(device)
    if num_modalities == 5:
        model = HFBSurv(10735, (256, 256), (20, 0, 20), (0.05, 0.3), 20, 0.1).to(device)
    if num_modalities == 6:
        model = HFBSurv(27564, (256, 256), (20, 0, 20), (0.05, 0.3), 20, 0.1).to(device)
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter+50+1)):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            x_protein = torch.cat((x_protein,x_mutation),2)
            
            
            if num_modalities == 1:
                fusion = x_clin
            if num_modalities == 2:
                fusion = torch.cat((x_clin,x_mirna),2)
            if num_modalities == 3:
                fusion = torch.cat((x_clin,x_mirna,x_pathr),2)
            if num_modalities == 4:
                fusion = torch.cat((x_clin,x_mirna,x_pathr,x_dnameth),2)
            if num_modalities == 5:
                fusion = torch.cat((x_clin,x_mirna,x_pathr,x_dnameth,x_protein),2)
            if num_modalities == 6:
                fusion = torch.cat((x_clin,x_mirna,x_pathr,x_dnameth,x_protein,x_genex),2)
