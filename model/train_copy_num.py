...
def train_copynum(opt,train_data,test_data,device):
    cudnn.deterministic = True
    torch.cuda.manual_seed_all(666)
    torch.manual_seed(666) 
    random.seed(666)
    model = HFBSurv(2227, (128, 64), (20, 0, 20), (0.2, 0.3), 20, 0.1).to(device)
    ...
    train_loader = DataLoader(dataset=train_data, batch_size=5, shuffle=True, drop_last=True)#,pin_memory=True,num_workers=10,collate_fn=custom_collate_fn)
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter+100+1)):
        ...
        if opt.measure or epoch == (opt.niter+opt.niter_decay - 1):
            ...
            loss_test, cindex_test, _, _ = test_copynum(opt, model, test_data, device)
...
def test_copynum(opt,model, test_data, device):
    model.eval()
