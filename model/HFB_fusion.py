...
class HFBSurv(nn.Module):
    def __init__(self, input_dims, hidden_dims, output_dims, dropouts, rank,fac_drop):
        ...
        self.encoder_path = SubNet(self.path_in, self.path_hidden,self.path_prob)
        self.encoder_cona = SubNet(self.cona_in, self.cona_hidden,self.cona_prob)
        ...
        self.encoder = nn.Sequential(encoder1, encoder2)
        self.classifier = nn.Sequential(nn.Linear(64, self.label_dim))
    ...
    def mfb(self, x1, x2, output_dim):
        ...
        fusion = fusion.view(-1, 1, output_dim, self.rank)
        fusion = torch.sum(fusion, 3).squeeze(1)
        fusion = torch.sqrt(F.relu(fusion)) - torch.sqrt(F.relu(-fusion))
        if fusion.dim() == 1:
            fusion = fusion.unsqueeze(0)  # Ensure it has at least 2 dimensions
        fusion = F.normalize(fusion, dim=-1)  # Explicitly normalize along the last dimension
        return fusion
