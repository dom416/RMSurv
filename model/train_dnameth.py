...
def train_dnameth(opt,train_data,test_data,device):
    cudnn.deterministic = True
    torch.cuda.manual_seed_all(666)
    torch.manual_seed(666)
    random.seed(666)
    model = HFBSurv(4931, (128, 64), (20, 0, 20), (0.2, 0.3), 20, 0.1).to(device)
    ...
    train_loader = DataLoader(dataset=train_data, batch_size=20, shuffle=True, drop_last=True)#,pin_memory=True,num_workers=10,collate_fn=custom_collate_fn)
    # batch_size=int(len(train_data)/10)
    metric_logger = {'train':{'loss':[], 'pvalue':[], 'cindex':[], 'surv_acc':[]},
                      'test':{'loss':[], 'pvalue':[], 'cindex':[], 'surv_acc':[]}}
    c_index_best = 0

    for epoch in tqdm(range(opt.epoch_count, opt.niter+100+1)):
        model.train()
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            pred, embeddings = model(x_dnameth) 
            # for SNN only
        ...
        if opt.measure or epoch == (opt.niter+opt.niter_decay - 1):
            ...
            loss_test, cindex_test, _, _ = test_dnameth(opt, model, test_data, device)
...
def test_dnameth(opt,model, test_data, device):
    model.eval()
...
def test_and_plot(opt,model, test_data, device):
    ...
    for batch_idx, batch in enumerate(test_loader):
        ...
        if 'clinical_data' not in batch:
            print(f"Missing 'clinical_data' in batch: {batch}")
            raise KeyError("Key 'clinical_data' is missing from the batch")
        x_clin = batch['clinical_data'].to(device)
        x_pathr = batch['pathology_report'].to(device)
        x_unis = batch['uni_slide_image'].to(device)
        x_senmo_1024 = batch['senmo_1024'].to(device)
