...
def train_genex(opt,train_data,test_data,device):
    cudnn.deterministic = True
    torch.cuda.manual_seed_all(666)
    torch.manual_seed(666)
    random.seed(666)
    model = HFBSurv(16829, (48, 256), (20, 0, 20), (0.05, 0.3), 20, 0.1).to(device)
    #model = SeNMo_Trg(input_dim=17255, hidden_layers=[1024, 512, 128, 48], dropout_rate=0.01).to(device)
    model.to(device) 
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter+100+1)):
        ...
        if opt.measure or epoch == (opt.niter+opt.niter_decay - 1):
            ...
            loss_test, cindex_test, _, _ = test_genex(opt, model, test_data, device)
...
def test_genex(opt,model, test_data, device):
    model.eval()
