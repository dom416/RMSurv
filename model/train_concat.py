...
def train_concat(opt,train_data,test_data,device):
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter+50+1)):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            fusion = torch.cat((x_clin,x_pathr,x_genex,x_dnameth,x_mirna,x_protein),2)
        ...
        if opt.measure or epoch == (opt.niter+opt.niter_decay - 1):
            ...
            loss_test, cindex_test, _, _ = test_concat(opt, model, test_data, device)
...
def test_concat(opt,model, test_data, device):
    model.eval()
