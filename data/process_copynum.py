...
luad = load_and_transpose('/home/daf6674/lambda-stack-with-tensorflow-pytorch/LUAD_fusion/TCGA.LUAD.sampleMap_Gistic2_CopyNumber_Gistic2_all_data_by_genes.tsv.gz')
lusc = load_and_transpose('/home/daf6674/lambda-stack-with-tensorflow-pytorch/LUAD_fusion/TCGA.LUSC.sampleMap_Gistic2_CopyNumber_Gistic2_all_data_by_genes.gz')
...
combined_hvgenes = variance_threshold_selector(combined, 0.05)

# Insert sample and dataset columns back into the dataset
combined_hvgenes.insert(0, 'sample', samples)
combined_hvgenes.insert(1, 'dataset', dataset)

# Separate the combined dataset back into LUAD and LUSC
luad_hvgenes = combined_hvgenes[combined_hvgenes['dataset'] == 'LUAD'].drop(columns=['dataset']).copy()
lusc_hvgenes = combined_hvgenes[combined_hvgenes['dataset'] == 'LUSC'].drop(columns=['dataset']).copy()
...
# Save the CNV data as embeddings for both datasets
save_data_by_case_id(luad_hvgenes, 'copy_number', 'LUAD')
save_data_by_case_id(lusc_hvgenes, 'copy_number', 'LUSC')
