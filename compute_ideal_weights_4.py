...
def compute_ideal_weights(
    ...
    Returns:
    - optimal_weights (numpy.ndarray): Array of calculated optimal weights for the modalities.
    """
    ...
    for batch in train_loader:
        ...
        all_predictions = torch.cat(all_predictions, dim=0)  # Shape: [num_modalities * num_cases, 20]
        # Compute the mean and standard deviation along the batch dimension (dim=0)
        mean_predictions = torch.mean(all_predictions, dim=0)  # [20,]
        std_predictions = torch.std(all_predictions, dim=0)    # [20,]
    ...
    for batch in test_loader:
        ...
        for modality in modalities_order:
            ...
            risk_scores = (- torch.sum(survival, dim=1)).detach().cpu().numpy()  # Shape: [num_cases,]
    ...
    def weight_objective_function(weights, modality_preds, censor, updated_survival_times):
        try:
            ...
            # Combine modality predictions by weighted sum across modalities
            # modality_preds shape: [num_cases, num_modalities, 20]
            combined_pred = np.tensordot(modality_preds, normalized_weights, axes=(1, 0))  # Shape: [num_cases, 20]
            ...
            # Compute risk scores
            risk_scores = -np.sum(survival, axis=1)  # Shape: [num_cases,]
    ...
    # Define bounds for weights: between 0 and 1 for each modality
    bounds = [(0, 1) for _ in range(len(modalities_order))]

    # Run Differential Evolution optimization
    result = differential_evolution(
        func=weight_objective_function,
        bounds=bounds,
        args=(modality_preds_array, censor, updated_survival_times),
        strategy='best1bin',
        maxiter=100,
        popsize=15,
        tol=1e-6,
    ...
    if verbose:
        ...
        for i, weight in enumerate(optimal_weights):
            print(f"  {modalities_order[i]}: Weight = {weight:.6f}")
